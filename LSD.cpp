...
/*----------------------------------------------------------------------------*/
/** Fatal printf, print a message to standard-printf output and exit.
 */

 /*----------------------------------------------------------------------------*/
 /** Doubles relative printf factor
  */
...
{
	/* check Parameters */
	if (out == NULL) printf("add_7tuple: invalid n-tuple input.");
	if (out->dim != 7) printf("add_7tuple: the n-tuple must be a 7-tuple.");

	/* if needed, alloc more tuples to 'out' */
	if (out->size == out->max_size) enlarge_ntuple_list(out);
	if (out->values == NULL) printf("add_7tuple: invalid n-tuple input.");
		...
		for (xx = reg[i].x - 1; xx <= reg[i].x + 1; xx++)
			for (yy = reg[i].y - 1; yy <= reg[i].y + 1; yy++)
					...
					isaligned(xx, yy, angles, *reg_angle, prec))
				{
					...
					*reg_angle = atan2approx(sumdy, sumdx);
				}
}
	...
	int** reg_img, int* reg_x, int* reg_y)
{
	...
	/* check Parameters */
	if (img == NULL || X <= 0 || Y <= 0) printf("invalid image input.");
	...
	*/
	...
	{
		if (region == NULL) printf("'region' should be a valid image.");
		*reg_img = region->data;
		if (region->xsize > (unsigned int)INT_MAX ||
			region->xsize > (unsigned int)INT_MAX)
			printf("region image to big to fit in INT sizes.");
	...
	if (out->size > (unsigned int)INT_MAX)
		printf("too many detections to fit in an INT.");
	*n_out = (int)(out->size);
	...
	int** reg_img, int* reg_x, int* reg_y)
{
	...
	float quant = 2.0;       /* Bound to the quantization printf on the
								  gradient norm.                                */
